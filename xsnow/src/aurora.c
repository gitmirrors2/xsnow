...
void aurora_computeparms(AuroraMap *a)
{
   ...
   int np = a->w/a->step;
   create_aurora_base(a->points, AURORA_POINTS, a->slant, AURORA_S, a->theta*M_PI/180.0, 0, np, &(a->z), &(a->nz));
   ...
   if(1)
   {
	 ...
	 if (d0 != d)
	 {
	    ...
	    {
	       P("Right side... %d %d %d %ld\n",a->z[i].x,a->nfuzz,a->lfuzz,sizeof(fuzz_t));
	       int jmax = i + f;
		  ...
		  double alpha = a->za[i]*(double)(jmax - j)/(double)(jmax - i);
		  if(j == i)
		     alpha *=0.5;
	    ...
	    else // edge on the left side
	    {
		  ...
		  a->fuzz[a->nfuzz].y = a->z[i].y;
		  double alpha = a->za[i]*(double)(j-jmin)/(double)(i-jmin);
		  if (j == i)
		     alpha *=0.5;
		  a->fuzz[a->nfuzz].a = alpha;
		  a->fuzz[a->nfuzz].h = a->zh[i];
...
// int nw: input, length of scratch array each element 2 doubles, e.g. 1000 (np*n should be enough)
